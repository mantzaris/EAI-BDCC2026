"""Persistent, locked GPU-job reservations for this explicitly authorized onset study."""
import argparse
import datetime
import fcntl
import json
...
def main():
    ...
    p.add_argument('--phase',choices=['core','budget'],required=True);p.add_argument('--source-sha',required=True)
    p.add_argument('command',nargs=argparse.REMAINDER);args=p.parse_args()
    command=args.command[1:] if args.command and args.command[0]=='--' else args.command
    c=json.loads(Path('configs/onset_graph_study.json').read_text())['resource']
    ledger=Path('results/onset_graph/compute_ledger.jsonl');ledger.parent.mkdir(parents=True,exist_ok=True)
    with ledger.open('a+') as out:
        ...
        phase_cap=c['max_core_gpu_seconds'] if args.phase=='core' else c['max_budget_gpu_seconds']
